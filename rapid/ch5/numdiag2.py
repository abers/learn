...
from PyQt5.QtWidgets import (QCheckBox, QDialog, QDialogButtonBox, QGridLayout, QLabel, QLineEdit,
    ...
    def apply(self):
        ...
        if thousands == decimal:
            QMessageBox.warning(self, "Format Error",
                                "The thousands separator and the decimal "
                                "marker must be different")
            self.thousandsEdit.selectAll()
            self.decimalMarkerEdit.setFocus()
            return
        if len(decimal) == 0:
            QMessageBox.warning(self, "Foramt Error",
